...
class AdminModule {
    ...
    setupEventListeners() {
        ...
        document.getElementById('product-form')?.addEventListener('submit', (e) => this.submitProductForm(e));

        // Proveedores
        document.getElementById('btn-new-proveedor')?.addEventListener('click', () => this.openProveedorModal());
        document.getElementById('proveedor-form')?.addEventListener('submit', (e) => this.submitProveedorForm(e));
        ...
        document.getElementById('best-seller-period')?.addEventListener('change', (e) => {
            const container = document.getElementById('best-sellers-container');
            if (e.target.value === 'especifico') {
                document.getElementById('best-seller-date-container').style.display = 'flex';
            } else {
                document.getElementById('best-seller-date-container').style.display = 'none';
            }
            if (container && !container.classList.contains('hidden')) {
        ...
        document.getElementById('lotes-info-period')?.addEventListener('change', (e) => {
            const container = document.getElementById('lotes-info-container');
            if (e.target.value === 'especifico') {
                document.getElementById('lotes-info-date-container').style.display = 'flex';
            } else {
                document.getElementById('lotes-info-date-container').style.display = 'none';
            }
            if (container && !container.classList.contains('hidden')) {
        ...
        document.getElementById('top-buyers-period')?.addEventListener('change', (e) => {
            const container = document.getElementById('top-buyers-container');
            if (e.target.value === 'especifico') {
                document.getElementById('top-buyers-date-container').style.display = 'flex';
            } else {
                document.getElementById('top-buyers-date-container').style.display = 'none';
            }
            if (container && !container.classList.contains('hidden')) {
        ...
        document.getElementById('providers-period')?.addEventListener('change', (e) => {
            const container = document.getElementById('providers-performance-container');
            if (e.target.value === 'especifico') {
                document.getElementById('providers-date-container').style.display = 'flex';
            } else {
                document.getElementById('providers-date-container').style.display = 'none';
            }
            if (container && !container.classList.contains('hidden')) {
        ...
        document.getElementById('top-buyers-prod-period')?.addEventListener('change', (e) => {
            const container = document.getElementById('top-buyers-per-product-container');
            if (e.target.value === 'especifico') {
                document.getElementById('top-buyers-prod-date-container').style.display = 'flex';
            } else {
                document.getElementById('top-buyers-prod-date-container').style.display = 'none';
            }
            if (container && !container.classList.contains('hidden')) {
    ...
    renderStats(stats) {
        document.getElementById('inversion-total').textContent = this.formatMoney(stats.inversion_total || 0);
        document.getElementById('venta-potencial').textContent = this.formatMoney(stats.venta_potencial || 0);
        document.getElementById('ganancia-proyectada').textContent = this.formatMoney(stats.ganancia_proyectada || 0);
        document.getElementById('total-productos').textContent = stats.total_productos || 0;
    }

    renderSalesResumen(sales) {
        document.getElementById('total-pedidos').textContent = sales.total_pedidos || 0;
        document.getElementById('total-unidades').textContent = sales.total_unidades || 0;
        document.getElementById('total-vendido').textContent = this.formatMoney(sales.total_vendido || 0);
        document.getElementById('ganancia-real').textContent = this.formatMoney(sales.ganancia_real || 0);
    }
    ...
    async handleRestockSubmit(e) {
        ...
        const cantidad = document.querySelector('#restock-form input[name="cantidad"]').value;
        const precioCompra = document.querySelector('#restock-form input[name="precio_compra_lote"]').value;
        const descripcion = document.querySelector('#restock-form input[name="descripcion"]').value || '';
    ...
    renderProveedores(proveedores) {
        ...
        if (proveedores.length === 0) {
            tbody.innerHTML = '<tr><td colspan="6" style="text-align: center;">No hay proveedores</td></tr>';
            return;
        }
    ...
    async loadPedidos() {
        try {
            const filtro = document.getElementById('pedidos-estado-filter')?.value || '';
            ...
            const filtrados = filtro ? pedidos.filter(p => (p.estado_pedido || '').toLowerCase() === filtro) : pedidos;
    ...
    renderPedidosAdmin(pedidos) {
        ...
        if (pedidos.length === 0) {
            tbody.innerHTML = '<tr><td colspan="7" style="text-align:center;color:#999">No hay pedidos con ese estado</td></tr>';
            return;
        }
        ...
        const html = pedidos.map(p => {
            ...
            const estilo = estadoColors[est] || { bg: '#f0f0f0', color: '#666', icon: '●' };
            const fechaRaw = p.fecha_pedido || p.fecha;
            const fecha = fechaRaw ? new Date(fechaRaw).toLocaleDateString('es-CO', { year:'numeric', month:'short', day:'numeric' }) : '-';
            const nombreCliente = p.nombre_usuario || p.usuario_nombre || p.id_usuario || '-';
            const btnConfirmar = est === 'pendiente'
    ...
    async confirmarPago(pedidoId) {
        if (!confirm(`¿Confirmar el pago del pedido #${pedidoId}? El cliente verá su pedido como pagado.`)) return;
        try {
            await API.put(`/pedidos/confirmar-pago/${pedidoId}`, {});
            this.showNotification(`✅ Pago del pedido #${pedidoId} confirmado`, 'success');
            this.loadPedidos();
        } catch (error) {
        ...
        try {
            ...
            if (!pedido.detalles || pedido.detalles.length === 0) {
                ...
                const detallesHtml = pedido.detalles.map(d => {
                    const imagen = (d.imagen_url && d.imagen_url.trim() !== '') 
                                   ? (d.imagen_url.startsWith('/') ? d.imagen_url.slice(1) : d.imagen_url) 
    ...
    renderLotes(lotes) {
        ...
        if (lotes.length === 0) {
            tbody.innerHTML = '<tr><td colspan="9" style="text-align: center;">No hay lotes activos</td></tr>';
            return;
        }
        ...
        const html = lotes.map(l => {
            ...
            const badgeClass = porAgotar ? 'badge badge-danger' : 'badge badge-success';
    ...
    renderLowStockReport(products, container) {
        if (products.length === 0) {
            container.innerHTML = '<p style="text-align: center; color: #27ae60;">✅ Todos los productos tienen stock adecuado</p>';
            return;
        }
    ...
    async loadTopBuyersReport(toggle = true) {
        try {
            ...
            const periodo = document.getElementById('top-buyers-period')?.value || 'dia';
    ...
    async loadBestSellersReport(toggle = true) {
        try {
            ...
            const periodo = document.getElementById('best-seller-period')?.value || 'dia';
            let query = `?periodo=${periodo}`;
            if (periodo === 'especifico') {
    ...
    async loadProvidersPerformanceReport(toggle = true) {
        try {
            ...
            const periodo = document.getElementById('providers-period')?.value || 'dia';
            let query = `?periodo=${periodo}`;
            if (periodo === 'especifico') {
            ...
            const data = await API.get(`/admin/reports/providers-performance${query}`);
    ...
    async loadTopBuyersPerProductReport(toggle = true) {
        try {
            ...
            const periodo = document.getElementById('top-buyers-prod-period')?.value || 'dia';
            let query = `?periodo=${periodo}`;
            if (periodo === 'especifico') {
            ...
            const data = await API.get(`/admin/reports/top-buyers-per-product${query}`);
            this.renderTopBuyersPerProductReport(data, container);
        } catch (error) {
            console.error('Error cargando reporte de cliente top por producto:', error);
            this.showNotification('Error al cargar reporte', 'error');
        }
    }
    ...
    async loadLotesInfoReport(toggle = true) {
        try {
            ...
            const periodo = document.getElementById('lotes-info-period')?.value || 'dia';
            let query = `?periodo=${periodo}`;
            if (periodo === 'especifico') {
    ...
    renderLotesInfoReport(data, container) {
        if (data.length === 0) {
            container.innerHTML = '<p style="text-align: center; color: #999;">No hay datos disponibles para esta fecha</p>';
            return;
        }
        const html = `
            <table class="report-table">
...
window.loadProvidersPerformanceReport = () => admin.loadProvidersPerformanceReport();
window.loadPaymentMethodsReport = () => admin.loadPaymentMethodsReport();
window.loadTopBuyersPerProductReport = () => admin.loadTopBuyersPerProductReport();
window.loadLotesInfoReport = () => admin.loadLotesInfoReport();

// Manejar el formulario de reabastecer stock
document.getElementById('restock-form')?.addEventListener('submit', (e) => admin.handleRestockSubmit(e));
